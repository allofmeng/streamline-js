#!/usr/bin/env python3
"""Build a self-contained copy of USER_MANUAL.md for publishing as an Artifact.

The repo copy uses relative image paths (media/foo.jpg) so it renders on GitHub.
A published Artifact has no origin to resolve those against, and its CSP blocks
external hosts anyway, so images must be inlined as data URIs.
...
def main():
    ...
    text, n = re.subn(r"!\[([^\]]*)\]\((media/[^)]+)\)", inline, text)
